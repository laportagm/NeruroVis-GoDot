...
rm -rf temp_syntax_check &>> cleanup_log.txt
rm -f temp_line_reader.py &>> cleanup_log.txt

# 2. Remove redundant backup directories
echo "Removing redundant backup directories..." | tee -a cleanup_log.txt
rm -rf copy3 &>> cleanup_log.txt

# 3. Remove backup files
echo "Removing backup files..." | tee -a cleanup_log.txt
rm -f project.godot.backup &>> cleanup_log.txt
rm -f scenes/*.bak &>> cleanup_log.txt
