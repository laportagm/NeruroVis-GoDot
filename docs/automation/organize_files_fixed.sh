...
# Move validation scripts to tools/scripts/
...
for file in test_*.gd; do
  # Skip files that we've already categorized
  if [[ "$file" == test_*ui* ]] || [[ "$file" == test_responsive* ]] || [[ "$file" == test_ai_* ]] || [[ "$file" == test_gemini_* ]] || [[ "$file" == test_syntax.gd ]]; then
    continue
  fi
