...
func register_model_with_switcher(model: Node3D, name: String) -> void:
	if ModelSwitcherGlobal:
		ModelSwitcherGlobal.register_model(model, name)
		print("ModelCoordinator: Registered model '" + name + "' with ModelSwitcher")
	else:
