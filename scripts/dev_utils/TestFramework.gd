	...
	if not debug_cmd:
		...
		if error_tracker:
			...
			if error_tracker.has_method("log_error"):
				error_tracker.log_error(1, 0, "Integration test")  # Use numeric values instead of enum
		else:
		...
		if health_monitor:
			...
			var health_status = health_monitor.get_health_status() if health_monitor.has_method("get_health_status") else {}
